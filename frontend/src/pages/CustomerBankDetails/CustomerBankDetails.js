...
const CreateCustomerBank = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append("UserID", user.CustomerID);

    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/customer-bank-accounts/",
        data,
        {
          headers: {
      ...
      console.log("Customer Bank Detail Created", response.data); // log the user data after signup

      // Store the user data in your context
      navigate("/login"); // Redirect the user to the next page
    } catch (error) {
      console.error(error);
    }
  };
